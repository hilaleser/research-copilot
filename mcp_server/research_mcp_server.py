...
# Minimum cosine similarity for a retrieved passage to count as evidence.
# Tuned against real results from all-MiniLM-L6-v2 on paper abstracts:
# a genuinely on-topic passage scored ~0.41, a loosely related one ~0.20,
# and unrelated papers sat around 0.15. 0.25 sits in the gap.
MIN_SIMILARITY = 0.25
...
def search_evidence(question: str, limit: int = 5,
    ...
    # Top-K alone is not enough. It always returns K rows, so when the corpus
    # holds nothing relevant it hands back the least-irrelevant thing it has -
    # and the agent then cites a paper that has nothing to do with the
    # question. Observed in testing: a question about attention returned a
    # paper on AI consciousness at 0.151 similarity, purely because it was in
    # the collection.
    #
    # A floor turns "here are five passages" into "here are the passages that
    # actually matched", which is a different and more honest claim.
    passages = [p for p in candidates if p["similarity"] >= MIN_SIMILARITY]

    if not passages:
        # Rather than returning nothing, hand back the single best match and
        # say plainly that it is weak. The agent can then tell the student the
        # collection does not cover this, instead of inventing an answer.
        best = candidates[0]
        return ok(
            "search_evidence",
            f"No passage matched well (best similarity {best['similarity']}, "
            f"below the {MIN_SIMILARITY} threshold). The collection may not "
            "cover this question - consider find_papers on this topic. "
            "Treat the passage below as weak evidence.",
            question=question,
            passages=[best],
            weak_match=True,
        )
    ...
    note = f" Filtered out {dropped} passage(s) below the relevance threshold." if dropped else ""

    return ok(
        "search_evidence",
        f"Found {len(passages)} relevant passages across "
        f"{len({p['openalex_id'] for p in passages})} papers.{note}",
        question=question,
        passages=passages,
    )
