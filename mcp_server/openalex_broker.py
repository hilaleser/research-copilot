...
def search_papers(query: str, limit: int = 10, min_year: int | None = None) -> list[dict[str, Any]]:
    ...
    #
    # has_abstract:true - a work without an abstract cannot be embedded, so it
    #   would be dead weight in the vector store and unusable as evidence.
    #
    # type:article - OpenAlex's /works endpoint returns more than papers. A
    #   search for "attachment in human psychology" came back with two book
    #   reviews, a book, and a record for the *Journal of Applied Social
    #   Psychology* itself. None of those are things a student reads as a
    #   paper, and a journal record in a reading list looks like a bug.
    #   Restricting to articles also improved topical relevance noticeably -
    #   Bowlby's "The Making and Breaking of Affectional Bonds" surfaced only
    #   once the non-articles were out of the way.
